...
bool Program_readLocalData(Object& obj, Input& fr)
{
    ...
    if(fr.matchSequence("GeometryVerticesOut %i"))
    {
        ...
        program.setParameter(GL_GEOMETRY_VERTICES_OUT, verticesOut);
        fr += 2;
    ...
    if(fr.matchSequence("GeometryInputType %w"))
    {
        ...
        if(Geometry_matchPrimitiveModeStr(primitiveMode.c_str(), mode))
            program.setParameter(GL_GEOMETRY_INPUT_TYPE, mode);
        fr += 2;
        iteratorAdvanced = true;
    }
    ...
    if(fr.matchSequence("GeometryOutputType %w"))
    {
        ...
        GLenum mode;
        if(Geometry_matchPrimitiveModeStr(primitiveMode.c_str(), mode))
            program.setParameter(GL_GEOMETRY_OUTPUT_TYPE, mode);
        fr += 2;
        iteratorAdvanced = true;
    }
...
bool Program_writeLocalData(const Object& obj,Output& fw)
{
    ...
    fw.indent() << "GeometryVerticesOut " << program.getParameter(GL_GEOMETRY_VERTICES_OUT) << std::endl;
    fw.indent() << "GeometryInputType " << Geometry_getPrimitiveModeStr(program.getParameter(GL_GEOMETRY_INPUT_TYPE)) << std::endl;
    fw.indent() << "GeometryOutputType " << Geometry_getPrimitiveModeStr(program.getParameter(GL_GEOMETRY_OUTPUT_TYPE)) << std::endl;
